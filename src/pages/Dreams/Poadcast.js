...
export default function Poadcast() {
  ...
  const onEnded = (current) => {
    if(isRandomAudio){
      setActiveAudio(audios[Math.trunc( Math.random() * (audios.length - 1) + 0)] )
  ...
  useEffect( () => { console.log({ isRandomAudio})}, [isRandomAudio])

  
  return (
    <main className='sm:flex sm:justify-center sm:items-center h-full w-full'>
      <section className='grid w-full h-full grid-cols-1 grid-rows-2 xl:grid-cols-2 xl:grid-rows-1'>
        {/* AUDIO PLAYER */}
        <div className='relative w-full h-full flex justify-center items-center order-5'>
          <div style={poadcastPlayerStyle} className='relative w-full h-full'>
            <div className='absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-8/12 h-auto flex flex-col justify-center items-center pl-4'>
              <img
                className='w-10/12'
              ...
              <div className='w-full'>
                <AudioPlayer
                  ...
                  customIcons={{
                    pause: <Pause className="h-10 w-10" />,
                    rewind: <Backward className="h-9 w-9" />,
                    forward: <Forward className="h-9 w-9" />,
                    next: <Loop className="h-7 w-7" />,
                    previous: <Shuffle className="h-7 w-7" />
                  }}
                />
              </div>
        ...
        {/* INFORMATION COL */}
        <div className='flex flex-col justify-center items-center order-5'>
          <img
            src={QueSoniamos}
