...
export default function CardMessage({ content }) {
  const {
    title, description, author, isCauseGallery = false,
  } = content;

  const style = isCauseGallery && dimensions.includes(content.index) ? 'mr-8' : 'mr-0';
  ...
  return (
    ...
    >
      <h1 className="text-justify text-base xl:text-xl font-semibold whitespace-nowrap pb-2">
        {title}
      </h1>
      <h2 className="text-justify text-sm lg:text-xs xl:text-sm 2xl:text-base font-light pb-2 w-full">
        {description}
      </h2>
      <p className="text-justify text-sm self-end font-medium 2xl:text-lg">
        {author}
      </p>
    </div>
  );
